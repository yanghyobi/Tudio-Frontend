...
const AWS_PUBLIC_IP = '3.34.194.219';
...
const VideoMeetingRoom = () => {
    ...
    const waitingRoomUrl = `http://localhost:8060/tudio/videoChat/waitingRoom/${roomId}`; // 대기실 URL 경로
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            try {
                // 1. 카메라/마이크를 소켓 연결 전에 '먼저' 완벽히 획득 (새로고침 방지 핵심)
    ...
    const connectSocket = () => {
        ...
        socket.on("connect", () => {
            ...
            socket.emit("join_room", {
                roomId: roomId, 
                userType: "MEETING" // 실제 화상미팅 참여자 임을 명시 : 대기실이 아닌 회의 화면에 접속한 상태
            });
            // ✅ 방 이름 가져오기 및 소켓 서버 전파
        ...
        // 1. 기존 유저 목록 받기 (새로 들어온 사람에게 발생)
        socket.on("all_users", (users) => {
            users.forEach(user => {
                if (user.socketId !== socket.id) {
                    // 새로 들어온 나는 기존 사람들에게 Offer를 보내지 않고 기다립니다 (false)
                    createPeerConnection(user.socketId, user.name, false);
                }
            });
        });

        // 2. 새 유저 입장 (방에 있던 사람들에게 발생)
        socket.on("user_joined", (user) => {
            if (user.socketId !== socket.id) {
                console.log(`🚪 새 유저 입장: ${user.name}`);
                // 방에 있던 사람이 새 유저에게 Offer를 보냄
    ...
    const createPeerConnection = (socketId, userName, shouldCreateOffer) => {
        ...
        pc.ontrack = (e) => {
            ...
            // if (userName === myName) return; 내 화면 중복 방지

            setPeers(prev => {
                // 중복 화면 해결 핵심: 같은 이름의 기존 정보가 있다면 제거하고 새 정보로 교체
    ...
    const fetchRoomNameAndSync = async (socket) => {
        try {
            ...
            // ✅ 에러 발생 시 사용자에게 알림 (CORS 또는 권한 문제)
    ...
    const handleEndCall = () => {
        swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // ✅ 창 닫기 실행 (target="_blank"로 열린 경우 작동)
                window.close(); 
                
                // 만약 팝업 차단 등으로 안 닫힐 경우를 대비한 대체 이동
                setTimeout(() => {
