...
const TodoWidget = ({ projectNo, projectName, onClearProject, refreshTrigger }) => {
    ...
    // UI 상태 관리
    const [isAdding, setIsAdding] = useState(false);     // 추가 입력창 표시 여부
    const [newTodo, setNewTodo] = useState('');          // 새 할 일 내용
    const [editingId, setEditingId] = useState(null);    // 수정 중인 투두 ID
    ...
    useEffect(() => {
        ...
        setEditingId(null);
        setFilterType('ALL');
    }, [projectNo]);    // 선택한 프로젝트 변경시 상태 초기화

    // WebSocket 신호(refreshTrigger)가 오면 목록 재조회
    useEffect(() => {
        if (refreshTrigger > 0) {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            if (res.data === 'OK') {
                ...
                setNewTodo('');
                // setIsAdding(false); 연속 입력을 위한 입력창 유지 설정
                fetchTodos(); 
                if(inputRef.current) inputRef.current.focus(); // 계속 입력 가능하게 포커스 유지
            } else {
    ...
    const handleToggle = async (todo) => {
        ...
        try {
            ...
            if (res.status === 200 && res.data === 'OK') {
                ...
                // 200은 왔지만 내용이 "FAIL"이거나 다른 경우 -> 실패로 간주
                throw new Error("DB 업데이트 실패 (0건 수정됨)");
            }
        } catch (error) {
            setTodos(prevTodos); // 실패 시 원복
    ...
    return (
        <div className="widget-content zen-folder-widget bg-white d-flex flex-column rounded shadow-sm position-relative" 
            ...
            {/* 폴더 모양 배경 레이어 (folder-shape-layer) */}
                ...
                {isAdding && (
                    <div className="flex-shrink-0 px-3 mb-2 pt-2 animate-fade-in">
                        <li className="list-group-item d-flex justify-content-between align-items-center bg-light border rounded p-2">
                            ...
                            {/* <button className="btn btn-sm btn-secondary" onClick={() => setIsAdding(false)}>취소</button> */}
                        </li>
                    </div>
                )}

                {/* 리스트 영역 (widget-content) */}
