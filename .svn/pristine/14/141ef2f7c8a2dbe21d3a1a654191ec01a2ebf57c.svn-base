...
const ApprovalWidget = ({ refreshTrigger }) => {
    const [data, setData] = useState({
        ...
        myDrafts: [],
        toApprove: [],
        isClient: false // true: 결재자(관리자), false: 기안자(일반)
    });
    ...
    // [2] 웹소켓 신호(refreshTrigger) 감지 시 재실행
    useEffect(() => {
        if (refreshTrigger > 0) {
    ...
    const handleTabClick = (tabName) => {
        if (activeTab === tabName) setActiveTab(null); // 토글
